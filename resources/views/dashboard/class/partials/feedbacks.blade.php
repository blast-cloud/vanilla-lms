      ...
      <div class="row">
        @if ($x->course_class_id == $courseClass->id)
          <div class="col-md-6">
            <dl>
                <dt class="mb-0">
                    <span class="text-danger" style="font-size:100%"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> New Feedback Request For {{$courseClass->code}} Class</span><br>
                ...
                <dd class="mb-0" style="font-size:85%;">
                    ...
                    $responses = $x->courseFeedbackResponse()
                                   ...
                                   ->count();
                    if($responses == 0){
                        $responses = "You have not responded to this feedback request.";
                    }elseif($responses == 1){
                        $responses = "You have responded to this feedback request.";
                    }else{
                        $responses = "You can't respond more than once.";
                    }
                    @endphp
            <i class="text-info fa fa-exclamation-triangle" aria-hidden="true"></i><strong><span class="text-info">{{ $responses }}</span></strong>             
            </dd>
           </dl>        
       </div>
...
@endforeach
    @if ($course_class_feedback_requests == 0)
        <p style="font-size:95%; text-align:center" class="muted text-danger">No Feedback Requests Available.</p>
    @endif
<hr class="light-grey-hr mb-10 mt-0"/>
...
<div class="row">
  @if ($x->course_class_id == $courseClass->id)
      <div class="col-md-6">
          <dl>
              <dt class="mb-0">
                  <span class="text-danger" style="font-size:100%"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Feedback Request Sent For {{$courseClass->code}} Class</span><br>
                  Start Date for Submission: <span id="spn_start_{{$x->id}}_date">{{ date('Y-m-d', strtotime($x->start_date)) }}</span><br>
                  Due Date for Submission: <span id="spn_due_{{$x->id}}_date">{{ date('Y-m-d', strtotime($x->end_date)) }}</span><br>
...
<div class="row">
  @if ($x->course_class_id == $courseClass->id)
      <div class="col-md-6">
          <dl>
              <dt class="mb-0">
                <span class="text-danger" style="font-size:85%"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> A Feedback was Requested for {{ $courseClass->code }} Class on {{ date('d-m-Y', strtotime($x->created_at))}}.</span><br><br>
                <span class="text-danger" style="font-size:85%"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Response from Students will be available after {{ date('d-m-Y', strtotime($x->end_date))}}.</span><br>
              </dt>        
         </dl>           
</div> 
<div class="col-md-6" style="text-align:right">   
